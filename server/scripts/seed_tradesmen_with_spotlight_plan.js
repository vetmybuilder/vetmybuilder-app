 ...
 *
 * Usage (env-driven, no sqlite DB file):
 *   MYSQL_HOST=localhost MYSQL_USER=root MYSQL_PASSWORD=... MYSQL_DATABASE=vetmybuilder \
 *     node server/scripts/seed_tradesmen_with_spotlight_plan.js --count=10
 */
...
// SQL-style "YYYY-MM-DD HH:MM:SS", e.g. 2025-11-14 13:01:54
const nowSql = () => {
  const d = new Date();
...
async function main() {
  ...
  console.log(
    `Seeding ${COUNT} spotlight tradesmen into MySQL database "${
      process.env.MYSQL_DATABASE || "vetmybuilder"
    }"...`
  );
  ...
  try {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      await connection.execute(
        ...
        ON DUPLICATE KEY UPDATE
          ...
          contact_credits = VALUES(contact_credits),
          ...
          plan = VALUES(plan),
          purchased_plan = VALUES(purchased_plan)
        `,
        [
          uid,
          company,
      ...
      for (let idx = 0; idx < PHOTO_URLS.length; idx++) {
        const url = PHOTO_URLS[idx];
        await connection.execute(
          `
          INSERT INTO ${PHOTOS_TABLE} (tradesman_user_id, url, sort_order, created_at)
          VALUES (?, ?, ?, ?)
          `,
          [uid, url, idx + 1, photosCreatedAt]
        );
      }
      ...
      if (existingRows.length === 0) {
        const createdAtOneoff = nowSql();
        const expiresAt = addDaysIso(30); // 30 days from now
        ...
        await connection.execute(
          ...
          VALUES (?, 'spotlight', NULL, 3999, 'GBP', 'active', ?, NULL, ?, ?)
          `,
          [uid, sessId, expiresAt, createdAtOneoff]
        );
      }
    }
    ...
    console.log(
      `✅ Seeded ${COUNT} spotlight tradesmen into MySQL database "${
        process.env.MYSQL_DATABASE || "vetmybuilder"
      }"`
    );
