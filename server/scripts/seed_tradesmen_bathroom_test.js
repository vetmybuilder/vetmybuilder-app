 ...
 *   node server/scripts/seed_tradesmen_bathroom_test.js
 */
...
// tables (match your existing naming)
...
async function main() {
  ...
  console.log(
    `ℹ️ Connected to MySQL ${MYSQL_DATABASE} at ${MYSQL_HOST}:${MYSQL_PORT}`
  );

  // Ensure minimal tables exist (no-op if already created by migrations)
  await conn.execute(`
  ...
  try {
    ...
    for (let idx = 0; idx < FIXTURES.length; idx++) {
      ...
      // photos: clear and insert
      await conn.execute(
        `DELETE FROM ${PHOTOS_TABLE} WHERE tradesman_user_id = ?`,
        [uid]
      );
      const photosCreatedAt = nowSql();
      for (let pIdx = 0; pIdx < PHOTO_URLS.length; pIdx++) {
        const url = PHOTO_URLS[pIdx];
        await conn.execute(
          `
          INSERT INTO ${PHOTOS_TABLE} (tradesman_user_id, url, sort_order, created_at)
          VALUES (?, ?, ?, ?)
        `,
          [uid, url, pIdx + 1, photosCreatedAt]
        );
      }

      // gold subscription (one per user)
      const alreadyGold = await hasGoldSubForUser(uid);
      if (!alreadyGold) {
        ...
        await conn.execute(
          ...
          VALUES (?, 'gold', ?, 'GBP', 'succeeded', ?, NULL, NULL, ?, ?)
        `,
          [uid, 2900, sessId, `mock:${sessId}`, createdSubAt]
        );
      }
    }
    ...
    console.log(
      `✅ Seeded ${seeded.length} bathroom-test tradesmen into MySQL database "${MYSQL_DATABASE}"`
    );
    seeded.forEach((r) => {
      console.log(
    ...
    await conn.rollback();
    console.error("❌ Error seeding bathroom test tradesmen:", err);
    process.exitCode = 1;
  } finally {
    await conn.end();
  }
