 ...
 *   - MYSQL_DATABASE (default: vetmybuilder)
 */
...
// warranty / scoring (same as original)
...
async function main() {
  ...
  console.log(
    `Seeding ${COUNT} gold tradesmen into MySQL database "${DB_NAME}"...`
  );
  ...
  try {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      // upsert into tradesmen (assumes `user_id` is PRIMARY KEY)
      ...
      // gold subscription row if missing
    ...
    const totalGold = countRows[0]?.c || 0;
    ...
    console.log(`✅ Seeded ${COUNT} gold tradesmen into ${DB_NAME}`);
    console.log(`Total gold tradesmen now: ${totalGold}`);
    ...
    console.log("✅ Subscriptions table used: payments_subscription");
    console.log("✅ Tradesmen photos table used: tradesmen_photos");
  } catch (err) {
