...
module.exports = (router, ctx) => {
  ...
  // detect photo table once per process
  const resolvePhotoTable = async () => {
    ...
    try {
      ...
      console.warn(
        "[tradesmen/favourites] resolvePhotoTable failed:",
        e?.message || e
      );
  ...
  const loadPhotosByBuilder = async (builderIds) => {
    const map = {};
    if (!builderIds || !builderIds.length) return map;
    ...
    try {
      ...
      console.warn(
        "[tradesmen/favourites] loadPhotosByBuilder failed:",
        e?.message || e
      );
      return map;
    }
  ...
  router.get("/tradesmen/favourites", auth, async (req, res) => {
    try {
      ...
      try {
        ...
        // If the favourites table doesn't exist yet, just return empty list
        if (e && (e.code === "ER_NO_SUCH_TABLE" || e.errno === 1146)) {
          console.warn(
            "[GET /tradesmen/favourites] favourite_tradesmen missing, returning empty list"
          );
          return res.json({ items: [] });
        }

        console.error(
          "[GET /tradesmen/favourites] SELECT failed",
          e?.message || e
        );
      ...
      const photoRowsByBuilder = await loadPhotosByBuilder(
