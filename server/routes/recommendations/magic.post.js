  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        // Load project (status + location + owner + name) from MySQL
        ...
        // 1) Try local DB exact match (case-insensitive) using MySQL
        ...
        // Notify owner via existing notifications helper (still SQLite-backed)
        try {
          if (proj.ownerUserId) {
            const submitter = uid || null;
            if (proj.ownerUserId !== submitter) {
              if (typeof notifyUsers === "function" && db) {
                notifyUsers(db, [proj.ownerUserId], {
                  type: "recommendation_new",
                  message: `Someone has recommended a tradesperson to your project “${proj.name}”`,
        ...
        // Auto-like when hireAgain != "no" (MySQL, no INSERT OR IGNORE)
        try {
          if (hireAgainRaw !== "no") {
            ...
            const existingRows = await mysqlQuery(
              `SELECT 1
                ...
                WHERE recommendationId = ? AND userId = ?
                LIMIT 1`,
