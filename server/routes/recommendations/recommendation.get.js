...
module.exports = (router, ctx) => {
  const { mysqlQuery, admin, extractLocationTokens } = ctx;
  if (!mysqlQuery) {
  ...
  router.get("/recommendations/:id", optionalAuth(admin), async (req, res) => {
    try {
      ...
      // main recommendation + project join
      ...
      const row = recRows && recRows[0] ? recRows[0] : null;
      ...
      if (uid) {
        try {
          ...
          myLike = likeRows && likeRows.length ? 1 : 0;
        } catch {
          myLike = 0;
        }
      }
      ...
      // fromCommunity badge (project vs recommender locality)
      let fromCommunity = 0;
      try {
        if (
      ...
      console.error("[GET /recommendations/:id] error", e);
