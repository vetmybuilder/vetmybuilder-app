// server/v2/routes/verify-company.get.js
/**
 * GET /api/v2/verify-company
 * Auth: required (server-side proxy to Companies House)
 * Query: companyNumber OR name[&postcode][&scored=1|strategy=match]
 */
module.exports = (router, ctx) => {
  const { auth, getCompanyProfile, searchCompanies, matchByName } = ctx;
  const { _roughNameScore } = require("../lib/companyVerifyHelpers");

  router.get("/verify-company", auth, async (req, res) => {
    try {
      const companyNumber =
