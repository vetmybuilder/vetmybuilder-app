...
import ProjectTabs, { type ProjectTabKey } from "@/components/ProjectTabs"; // mobile fallback
...
import ProjectFilters, { type ProjectFiltersValue } from "@/components/filters/ProjectFilters";
...
type Project = {
  ...
  // fields used to render/link tradesman
...
const VERTICAL_TABS = [
  ...
  },
  {
    ...
    icon: (p: React.SVGProps<SVGSVGElement>) => (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...p}>
        <path d="M12 3l7 3v5c0 5-3.5 8-7 10-3.5-2-7-5-7-10V6l7-3z" strokeWidth="1.75" />
        <path d="M8.5 12.5l2.5 2.5 4.5-5" strokeWidth="1.75" strokeLinecap="round" />
      </svg>
    ),
  ...
  },
  {
    ...
    icon: (p: React.SVGProps<SVGSVGElement>) => (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" {...p}>
        ...
        <path d="M4.5 17c.6-2 2.8-3.5 5.5-3.5S15 15 15.5 17" strokeWidth="1.75" />
        <rect x="15.5" y="4" width="5" height="4" rx="0.75" strokeWidth="1.75" />
        <path d="M16.5 6l1.2 1.2L20 5" strokeWidth="1.75" strokeLinecap="round" />
      </svg>
    ),
...
// ---- helpers (from the pre-redesign logic) ----
const normalizeHookLabel = (v: unknown) => {
  const s = (typeof v === "string" ? v : "").trim();
  if (!s) return "";
  if (/^#\d+$/i.test(s)) return ""; // placeholders like "#123"
  if (s === "—" || s === "=") return "";
  return s;
};
...
export default function ProjectsPage() {
  ...
  function getUrlTab(): ProjectTabKey | undefined {
    ...
    return t && allowedTabs.has(t as ProjectTabKey) ? (t as ProjectTabKey) : undefined;
  }
  ...
  // Sorting (still sent to API)
  const [sort, setSort] = useState<"createdAt" | "name">("createdAt");
  const [order, setOrder] = useState<"asc" | "desc">("desc");

  // ===== Progressive loading state =====
  const PAGE_SIZE = 12; // tweak if you want (desktop ≈ 4x3)
  const [items, setItems] = useState<Project[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [hasMore, setHasMore] = useState(true);
  const [loading, setLoading] = useState(true);
  const [announce, setAnnounce] = useState(""); // a11y live region

  // Reset list when inputs change
  useEffect(() => {
    setItems([]);
    setTotal(0);
    setPage(1);
  ...
  async function fetchPage(p = 1) {
    ...
    try {
      ...
      setTotal(nextTotal);
      const totalSoFar = (p === 1 ? newItems.length : items.length + newItems.length);
      setHasMore(totalSoFar < nextTotal);
      setPage(p);
      setAnnounce(`Loaded ${totalSoFar} of ${nextTotal} projects`);
    } catch {
      // on error keep previous items; mark no more if first page empty
      if (p === 1) {
        setItems([]);
        setTotal(0);
        setHasMore(false);
  ...
  // Initial + whenever reset happens
  ...
  }, [hasMore, loading, page, inputsKey]); // rebind if inputs change

  const isCompletedLikeView = tab === "completed" || tab === "completedCommunity";

  // Resolve tradesman labels for completed rows (based on currently rendered items)
  const { labels: trades } = useTradesmanLabels(isCompletedLikeView, (items as any[]) || [], api);

  // ---- Dynamic options from the CURRENT items (same behavior as before) ----
  const { typeOptions, statusOptions } = useMemo(() => {
    ...
    const orderMap: Record<Status, number> = { live: 0, pending: 1, completed: 2, archived: 3 };
    const statusOptions = Array.from(statuses).sort((a, b) => orderMap[a] - orderMap[b]);
  ...
  const toggleSort = (col: "name" | "createdAt") => {
    ...
    // grid uses updated inputs → reset happens via effect
  };
          ...
          return (
            <li key={t.key}>
              ...
              >
                ...
                />
                <t.icon className="h-[18px] w-[18px] mt-0.5 shrink-0" aria-hidden />
  ...
  // ---- Click handler to open builder profile (pulled from working file) ----
  async function openBuilderProfile(p: Project) {
    ...
    try {
      const { data: closure } = await api.get<any>(`/api/projects/${p.id}/closure`);
      const rid =
  ...
  return (
    <AuthedOnly>
      {/* Reduced width & left margin so global menu + rail never overlap */}
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 xl:ml-[260px]" data-testid="projects-page">
        {/* Left text-only tabs (desktop) */}
        <VerticalTabs />

        {/* Mobile/Tablet horizontal tabs */}
        <div className="xl:hidden mb-3">
          <ProjectTabs value={tab} onChange={setTab} orientation="horizontal" className="[&_*]:text-[14px]" />
        </div>
        ...
        <ProjectFilters
          ...
          statusOptions={statusOptions as any} // Status[]
          items={items as any} // used for counts only
        ...
        <div className="mt-2" data-testid="projects-list">
          {isCompletedLikeView ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5" data-testid="projects-card-grid-completed">
              {items.map((p) => {
                const recId = (p as any)._winnerRecommendationId;
                const fromServer = normalizeHookLabel((p as any)._winnerTradesmanName);
                const fromHook = normalizeHookLabel((trades as any)?.[recId]);
              ...
              {items.length === 0 && !loading && (
                <div className="col-span-full text-sm text-zinc-400" data-testid="projects-empty">
                  No projects.
                </div>
              )}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5" data-testid="projects-card-grid">
              {items.map((p) => (
