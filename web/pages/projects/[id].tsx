...
export default function ProjectView() {
  ...
  const onPublish = async () => {
    ...
    try {
      ...
      setFlash({ kind: "error", text: e?.response?.data?.error || "Failed to publish" });
    } finally {
      setBusy(false);
    }
  };
  ...
  const onArchive = async () => {
    ...
    try {
      ...
      setFlash({ kind: "error", text: e?.response?.data?.error || e?.message || "Failed to archive" });
    } finally {
      setBusy(false);
    }
  };
  ...
  const onUnarchive = async () => {
    ...
    try {
      ...
      setFlash({ kind: "error", text: e?.response?.data?.error || e?.message || "Failed to unarchive" });
    } finally {
      setBusy(false);
    }
  };
  ...
  const onCopyInvite = async () => {
    ...
    try {
      ...
      setFlash({ kind: "success", text: copied ? "Invite link copied to clipboard" : `Invite link: ${data.url}` });
    } catch (e: any) {
      setFlash({ kind: "error", text: e?.response?.data?.error || e?.message || "Failed to generate link" });
    } finally {
      setBusy(false);
    }
  };

  const canShowAddFavourite = isFromCommunity && !isOwner && !isArchived && !addedToFavourites;
  const onAddToFavourites = async () => {
    ...
    try {
      ...
      setFlash({ kind: "error", text: e?.response?.data?.error || e?.message || "Could not favourite" });
    } finally {
      setBusy(false);
    }
  };
  ...
  async function loadRecs(pid: number) {
    const { data } = await api.get(`/api/projects/${pid}/recommendations?page=1&pageSize=3`);
    setRecs(data.items || []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const msg = e?.data?.error ?? e?.response?.data?.error ?? e?.message ?? "";
  ...
  useEffect(() => {
    ...
    (async () => {
      const entries = await Promise.all(
        recs.map(async (r) => {
          try {
            const { data } = await api.get(`/api/recommendations/${r.id}/verification`);
            ...
            return [r.id, { recommendationId: r.id, status: "error" as VerificationStatus }] as const;
          }
        })
      );
      if (!cancelled) {
  ...
  const likeOnce = async (rec: Recommendation) => {
    ...
    setLikingId(rec.id);
    setRecs((prev) =>
      (prev || []).map((r) => (r.id === rec.id ? { ...r, myLike: 1 as 0 | 1, likes: (r.likes ?? 0) + 1 } : r))
    );
    try {
    ...
    <AuthedOnly>
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8" data-testid="project-view-page">
        {(authLoading || (router.isReady && loading)) && (
          <p className="py-10 text-sm text-slate-500" data-testid="project-loading">Loading…</p>
        )}
        ...
        {!loading && errorStatus === 401 && (
          <EmptyState
            ...
            actions={
              <Link href="/login" className="btn mt-3" aria-label="Go to sign in" data-testid="btn-go-signin">
                Go to sign in
              </Link>
            }
          ...
          <EmptyState
            ...
            actions={
              <div className="flex gap-3">
                <Link href="/projects" className="btn" data-testid="btn-back-to-my-projects">
                  Back to My Projects
                </Link>
              </div>
            }
        ...
        {!loading && errorStatus != null && errorStatus !== 401 && errorStatus !== 404 && (
          <EmptyState
                ...
                onClick={() => {
                  if (projectId) {
                    ...
                    api
                      ...
                      .catch((e: any) => setErrorStatus(e?.status ?? e?.response?.status ?? 500))
        ...
        {!loading && !errorStatus && project && (
          <>
            <div className="mb-6 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur px-6 py-5 shadow-sm" data-testid="project-header">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div aria-labelledby="project-title">
                  <h1 id="project-title" className="text-2xl font-semibold tracking-tight" data-testid="project-title">
                    {project.name}
                  </h1>
                  <p className="mt-1 text-sm text-slate-500" aria-live="polite" data-testid="project-created">
                    Created {new Date(project.createdAt).toLocaleString()}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2" aria-label="Project actions" data-testid="project-actions">
                  <Link
                  ...
                  >
                    <svg viewBox="0 0 24 24" className="icon-24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Left: details */}
              <section className="lg:col-span-6" aria-labelledby="details-heading" data-testid="project-details">
                <div className="card">
                  {!!flash && (
                    <div
                      role="alert"
                      aria-live="assertive"
                      className={`mb-3 rounded-lg px-3 py-2 text-sm ${flash.kind === "success" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"} transition-opacity`}
                  ...
                  {isOwner && (
                    <div className="mb-4 flex flex-wrap gap-2" aria-label="Owner actions" data-testid="owner-actions">
                      <Link className="btn" href={`/projects/${project.id}/edit`} aria-label="Edit project" data-testid="btn-edit">Edit</Link>
                      {canPublish && (
                        <button className="btn" onClick={onPublish} aria-label="Publish project" disabled={busy} aria-busy={busy} data-testid="btn-publish">
                          Publish
                        </button>
                      )}
                      {!isArchived ? (
                        <button className="btn-danger" onClick={onArchive} aria-label="Archive project" disabled={busy} aria-busy={busy} data-testid="btn-archive">
                          {busy ? "Archiving…" : "Archive"}
                        </button>
                      ) : (
                        <button className="btn" onClick={onUnarchive} aria-label="Unarchive project" disabled={busy} aria-busy={busy} data-testid="btn-unarchive">
                          {busy ? "Unarchiving…" : "Unarchive"}
                        </button>
                      )}
                      {isLive && (
                        <button className="btn" onClick={onCopyInvite} aria-label="Copy invite link" data-testid="btn-copy-invite">
                          Copy Invite Link
                        </button>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4" role="list" aria-label="Project attributes" data-testid="project-badges">
                    <span role="listitem" className="badge blue" aria-label={`Type: ${project.type}`} data-testid="badge-type">{project.type}</span>
                    <span role="listitem" className="badge gray" aria-label={`Location: ${project.location}`} data-testid="badge-location">{project.location}</span>
                    <span role="listitem" className="badge orange capitalize" aria-label={`Property: ${project.propertyType}`} data-testid="badge-property">{project.propertyType}</span>
                    <span role="listitem" className="badge green" aria-label={`Bedrooms: ${project.bedrooms}`} data-testid="badge-bedrooms">{project.bedrooms} bed</span>
                    <span role="listitem" aria-label={`Project ${project.status}`} data-testid="badge-status"><StatusBadge value={project.status} /></span>
                  </div>
                  ...
                  <dl className="grid grid-cols-2 gap-4" data-testid="project-fields">
                    <div>
                      ...
                      <dd className="font-medium" data-testid="field-type">{project.type}</dd>
                    </div>
                    <div>
                      <dt className="text-slate-500 text-sm">Location</dt>
                      <dd className="font-medium" data-testid="field-location">{project.location}</dd>
                    </div>
                    <div>
                      <dt className="text-slate-500 text-sm">Property</dt>
                      <dd className="font-medium capitalize" data-testid="field-property">{project.propertyType}</dd>
                    </div>
                    <div>
                      <dt className="text-slate-500 text-sm">Bedrooms</dt>
                      <dd className="font-medium" data-testid="field-bedrooms">{project.bedrooms}</dd>
                    </div>
                    <div>
                      <dt className="text-slate-500 text-sm">Status</dt>
                      <dd className="font-medium capitalize" data-testid="field-status"><StatusBadge value={project.status} /></dd>
                    </div>
                  </dl>

                  <div className="mt-5" data-testid="project-description-block">
                    <h3 className="font-semibold mb-1" id="desc-heading">Description</h3>
                    <p className="whitespace-pre-wrap text-slate-700" aria-labelledby="desc-heading" data-testid="field-description">
                      {project.description}
                    </p>
                  </div>
                </div>
                ...
                {canAddRec && !isOwner && !isArchived && (
                  <div className="mt-4">
                    <Link className="btn" href={`/projects/${project.id}/recommend`} aria-label="Add recommendation" data-testid="btn-add-recommendation">
                      Add recommendation
                    </Link>
                  </div>
                )}
              ...
              {/* Right: shortlist */}
              <aside className="lg:col-span-6" aria-labelledby="shortlist-heading" data-testid="project-shortlist">
                <div className="card">
                  <div className="flex items-start justify-between">
                    <div>
                      <h2 id="shortlist-heading" className="text-2xl font-semibold tracking-tight">Shortlist</h2>
                      <p className="mt-1 text-sm text-slate-500">Top 3 recommendations, ranked by the community.</p>
                    </div>
                  </div>
                  ...
                  {recsErr ? (
                    <p className="text-sm text-red-600" data-testid="shortlist-error">{recsErr}</p>
                  ) : !recs || recs.length === 0 ? (
                    <p className="text-sm text-slate-500" data-testid="shortlist-empty">No builders have yet been recommended.</p>
                  ) : (
                    <>
                      <ul className="space-y-3" aria-label="Top recommendations" data-testid="shortlist-list">
                        {recs.map((r) => {
                          ...
                          return (
                            <li key={r.id} className="rounded-xl border border-slate-200 bg-white/80 hover:bg-white shadow-sm hover:shadow-md transition p-3" data-testid="shortlist-item">
                              <div className="flex items-start gap-3">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-3">
                                    <div className="font-medium truncate flex-1 min-w-0" data-testid="shortlist-company">
                                      <Link href={`/builders/${r.id}`} className="hover:underline decoration-indigo-400/60" title="Open builder profile">
                                        {r.company}
                                      </Link>
                                    </div>
                                    <div className="shrink-0 flex items-center gap-3 whitespace-nowrap">
                                      <StarRating value={stars} />
                                      <div className="text-xs text-slate-500 tabular-nums flex items-center gap-1" aria-label={`${likes} likes`} data-testid="shortlist-likes">
                                        <LikeIcon className="h-3.5 w-3.5" /> {likes}
                                      </div>
                                    </div>
                                  </div>

                                  {r.comment && (
                                    <p className="text-sm text-slate-700 mt-1 line-clamp-3" data-testid="shortlist-comment">
                                      {r.comment}
                                    </p>
                                  )}
                                  ...
                                  <div className="mt-2 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span
                                        className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs ${chBadgeClass(vStatus)}`}
                                        title={`Companies House status${
                                          ver?.checkedAt ? ` · checked ${new Date(ver.checkedAt).toLocaleString()}` : ""
                                        }${ver?.companyNumber ? ` · ${ver.companyNumber}` : ""}`}
                                      ...
                                      {r.fromFriend ? (
                                        <span className="badge blue" aria-label="From a friend" data-testid="shortlist-badge-friend">Friend</span>
                                      ) : null}
                                      {r.fromCommunity ? (
                                        <span className="badge green" aria-label="From the community" data-testid="shortlist-badge-community">Community</span>
                                      ) : null}
                                      {hasPhotos && (
                                        <span
                                    ...
                                    <div className="text-xs text-slate-500" data-testid="shortlist-date">
                                      {new Date(r.createdAt).toLocaleDateString()}
                                    </div>
                                  </div>

                                  <div className="text-xs text-slate-500 mt-1" aria-label="Recommender" data-testid="shortlist-recommender">
                                ...
                                {!isOwner && (
                                  <div className="ml-3 shrink-0 flex flex-col items-center">
                                    <button
                                      onClick={() => likeOnce(r)}
                                      disabled={!user || hasLiked || likingId === r.id}
                                      className={`h-9 w-9 rounded-full grid place-items-center border transition
                                        ${hasLiked ? "bg-indigo-50 border-indigo-200 text-indigo-600 cursor-default" : "border-slate-200 hover:bg-slate-50"}
                                        ${!user ? "opacity-60" : ""}`}
                                      title={!user ? "Sign in to like" : hasLiked ? "You’ve liked this" : "Like"}
                                      aria-label={!user ? "Sign in to like" : hasLiked ? "You have liked this" : "Like"}
                                      data-testid="shortlist-like-button"
                                    >
                                      <LikeIcon className="h-4 w-4" />
                                    </button>
                                    <div className="mt-1 text-xs tabular-nums text-slate-600" aria-live="polite" data-testid="shortlist-like-count">
                                      {likes}
                                    </div>
                                  </div>
                                )}
                      ...
                      {recTotal > (recs?.length ?? 0) && (
                        <div className="mt-3">
                          <Link className="btn" href={`/projects/${project.id}/shortlist`} aria-label="View more recommendations" data-testid="btn-shortlist-view-more">
                            View more
                          </Link>
                        </div>
                      )}
...
function EmptyState({
  ...
  return (
    <div className="mx-auto max-w-3xl p-6" data-testid={dataTestId ?? "empty-state"} aria-live="polite" role="status">
      <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
        <p className="text-red-600 font-semibold" data-testid="empty-state-title">{title}</p>
        {description && (
          <p className="mt-2 text-slate-700" data-testid="empty-state-description">
            {description}
          </p>
        )}
